...
Figure \@ref(fig:resultfigure1) shows a line graph figure produced by one of the R scripts.

```{r resultfigure1,  fig.cap='Analysis figure.', echo=FALSE}
knitr::include_graphics("../../results/resultfigure1.png")
```

Figure \@ref(fig:resultfigure2) shows a line graph figure produced by one of the R scripts.
...
knitr::kable(resulttable, caption = 'LM fit for X7.day_avg_group_cases_per_100k and Administered_Dose1_pct_agegroup.')
```

Example table \@ref(tab:resulttable) shows a table summarizing a linear model fit.

```{r resulttableCD,  echo=FALSE}
resulttable=readRDS("../../results/resulttable1.rds")
knitr::kable(resulttable, caption = 'LM fit for X7.day_avg_group_cases_per_100k and Series_Complete_Pop_pct_agegroup.')
```
